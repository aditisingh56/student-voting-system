...
const VOTING_END_TIME =
  VOTING_START_TIME + VOTING_DURATION_HOURS * 60 * 60 * 1000;
...
const useCountdown = (endTime) => {
  ...
  useEffect(() => {
    const votingEndTime =
      parseInt(localStorage.getItem("votingEndTime")) || endTime;
...
function Dashboard({
  ...
  useEffect(() => {
    if (user.isAdmin) {
      ...
      const participation =
        totalUsers > 0 ? ((votedUsers / totalUsers) * 100).toFixed(1) : 0;
...
function VotingPage({ user, onVoteSubmit, onBackToDashboard }) {
  ...
  useEffect(() => {
    ...
    const departmentCandidates = allCandidates.filter(
      (candidate) =>
        candidate.department.toLowerCase() === user.department.toLowerCase()
    );
    ...
    const uniquePositions = [
      ...new Set(departmentCandidates.map((c) => c.position)),
    ];
    ...
    <div className="voting-container">
      ...
      <div>
        ...
        <div className="vote-submit">
          <button className="btn-primary large-btn" onClick={handleSubmitVote}>
            Submit Vote
          </button>
        </div>
      </div>
...
function ResultsPage({ user, onBackToDashboard }) {
  ...
  useEffect(() => {
    ...
    const total = allCandidates.reduce(
...
function VotingApp() {
  ...
  const handleVoteSubmit = (selectedCandidates) => {
    ...
    const updatedCandidates = allCandidates.map((candidate) => {
      const isSelected = Object.values(selectedCandidates).includes(
        candidate.id
      );
      return isSelected
    ...
    const updatedUsers = users.map((user) =>
      user.studentId === currentUser.studentId
        ? {
            ...user,
            hasVoted: true,
            votedCandidates: Object.values(selectedCandidates),
          }
        : user
    );
    localStorage.setItem("users", JSON.stringify(updatedUsers));
